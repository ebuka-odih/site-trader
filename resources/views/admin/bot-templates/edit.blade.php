...
<div class="p-4 bg-gray-50 dark:bg-gray-900 min-h-screen">
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
            ...
            <p class="text-gray-600 dark:text-gray-400 mt-1">{{ $template->name }}</p>
        </div>
    ...
    <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-sm p-6">
        ...
        <form method="POST" action="{{ route('admin.bot-templates.update', $template) }}" class="space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="flex flex-col sm:flex-row sm:justify-end gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
                ...
                <button type="submit" class="inline-flex items-center justify-center px-6 py-2 text-sm font-semibold text-white bg-indigo-600 rounded-lg hover:bg-indigo-700">Update Template</button>
            </div>
        </form>
    </div>
</div>
